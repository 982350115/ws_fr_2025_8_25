...
#include <geometry_msgs/msg/point_stamped.hpp> // 新增：用于订阅小球坐标
...
private:
    ...
    rclcpp::Subscription<geometry_msgs::msg::PointStamped>::SharedPtr ball_pose_sub_; // 新增：小球坐标订阅器
    ...
    std::atomic<bool> ball_pose_received_{false}; // 新增：标志，指示是否接收到有效小球坐标

    // 接收到的小球坐标（世界坐标系）
    geometry_msgs::msg::Pose current_ball_pose_;
    std::mutex ball_pose_mutex_; // 保护 current_ball_pose_ 的互斥锁
    ...
    const double ee_to_grasp_offset_Z_ = 0.175; 
    const std::string gazebo_model_name_ = "target_ball"; 

    // 小球和放置点
    const double ball_radius_ = 0.025;
    // const geometry_msgs::msg::Pose ball_pose_ = [](){ // **移除硬编码的 ball_pose_ 初始化**
...
private:
    ...
    void ball_pose_callback(const geometry_msgs::msg::PointStamped::SharedPtr msg)
    {
        ...
        // 接收到的 PointStamped 已经是 target_frame_ (即 world_frame_) 中的坐标
        ...
        // 小球检测只提供位置，姿态保持夹爪所需的固定姿态（竖直向下，w=1.0）
        current_ball_pose_.orientation.x = 0.0;
        current_ball_pose_.orientation.y = 0.0;
        current_ball_pose_.orientation.z = 0.0;
        current_ball_pose_.orientation.w = 1.0;
        
        if (!ball_pose_received_.load()) {
            ball_pose_received_ = true;
            RCLCPP_INFO(this->get_logger(), "Initial ball pose received in %s frame: (%.3f, %.3f, %.3f)",
    ...
    void initialize_and_start()
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Subscribing to ball pose from /ball_detector/ball_pose.");
    ...
    void addTableAndBallToPlanningScene()
    {
        ...
        // 2. 添加小球作为碰撞物体 (使用一个初始占位位姿，等待订阅到真实位姿后再更新)
        ...
        // 使用 place_pose_ 作为初始占位位姿，等待真实坐标到来
        ...
        RCLCPP_INFO(this->get_logger(), "Added target_ball collision object to planning scene (using placeholder pose).");
        
        // 3. 将 Gazebo 中的小球模型瞬移到初始抓取位置 (在 pickPlaceSequence 中，等待坐标到来后再执行)
    }
    ...
    {
        // --- 1. 等待小球坐标 ---
        RCLCPP_INFO(this->get_logger(), "Waiting for ball position from detector...");
        while (rclcpp::ok() && !ball_pose_received_.load()) {
        ...
        // 更新 MoveIt 场景中小球的真实位置，并瞬移 Gazebo 中的小球到该位置
        moveit_msgs::msg::CollisionObject ball_co;
        ball_co.id = "target_ball";
        ball_co.header.frame_id = move_group_->getPlanningFrame();
        shape_msgs::msg::SolidPrimitive sph;
        ...
        sph.dimensions[0] = ball_radius_;
        ball_co.primitives.push_back(sph);
        ball_co.primitive_poses.push_back(ball_pose_copy); // **使用接收到的坐标**
        ball_co.operation = ball_co.MOVE; // 移动到正确位置
        planning_scene_interface_->applyCollisionObjects({ball_co});
        RCLCPP_INFO(this->get_logger(), "Updated target_ball collision object with detected pose.");
        
        setGazeboBallPoseOnce(ball_pose_copy); // 瞬移 Gazebo 模型到检测到的位置
        rclcpp::sleep_for(500ms); // 给予模型足够时间稳定
        
        RCLCPP_INFO(this->get_logger(), "Starting FULL pick-and-place sequence (WITH REAL-TIME GAZEBO FOLLOW).");

        // 预抓取位姿计算 (基于订阅到的坐标)
        geometry_msgs::msg::Pose pre_grasp_pose = ball_pose_copy; // **使用接收到的坐标**
        ...
        grasp_pose.position.z = ball_pose_copy.position.z + ee_to_grasp_offset_Z_; // **使用接收到的坐标**
