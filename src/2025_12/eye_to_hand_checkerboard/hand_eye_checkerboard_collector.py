...
ROBOT_EE_FRAME = 'flange'                     # 机械臂末端 Frame (标定板所绑定的连杆)
# =========================================================================
...
class HandEyeCheckerboardCollector(Node):
    def __init__(self):
        ...
        print("---------------------------------------------------------")
        print("操作指南 (Eye-to-Hand 眼在手外 模式):")
        print("1. 移动带有标定板的机械臂末端，确保固定相机的画面中完整出现彩色角点连线及坐标轴。")
    ...
    def image_callback(self, msg):
        try:
            ...
            cv2.imshow("Eye-to-Hand Calibration View", display_img)
            cv2.waitKey(1)
    ...
    def save_data(self):
        filename = 'eye_to_hand_data.json'
